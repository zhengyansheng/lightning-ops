...
class OperateInstance:
    ...
    def get_table_relation(pid, cid):
        ...
        table_relation_obj = TableRelation.objects.filter(parent_table_id=pid,
                                                          child_table_id=cid).first()
        if table_relation_obj:
            return table_relation_obj
        return None

    @staticmethod
    def get_abs_asset_relation(t_id, p_id, c_id):
        """根据 table_relation_id parent_asset_id child_asset_id 查询asset_relation记录"""
        asset_relation = AssetsRelation.objects.filter(table_relation_id=t_id, parent_asset_id=p_id,
                                                       child_asset_id=c_id).first()
        if asset_relation:
    ...
    def get_child_asset_relation(t_id, c_id):
        ...
        asset_relation = AssetsRelation.objects.filter(table_relation_id=t_id, child_asset_id=c_id)
        if asset_relation:
            return asset_relation
        return None
